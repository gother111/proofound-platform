...
CREATE INDEX IF NOT EXISTS idx_verification_requests_email ON verification_requests(verifier_email)
...
CREATE INDEX IF NOT EXISTS idx_verification_rate_limit ON verification_requests(profile_id, created_at) 
...
DROP POLICY IF EXISTS "requester_reads_own_requests" ON verification_requests
...
DROP POLICY IF EXISTS "verifier_reads_by_token" ON verification_requests
...
DROP POLICY IF EXISTS "requester_creates_requests" ON verification_requests
...
DROP POLICY IF EXISTS "verifier_responds" ON verification_requests
...
DROP POLICY IF EXISTS "requester_updates_own_requests" ON verification_requests
...
DROP POLICY IF EXISTS "requester_deletes_own_requests" ON verification_requests
...
-- Prevent cancelling requests that are no longer pending
CREATE OR REPLACE FUNCTION prevent_invalid_verification_status_change()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.status = 'cancelled' AND OLD.status <> 'pending' THEN
    RAISE EXCEPTION 'Cannot cancel a verification request that is not pending';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql

DROP TRIGGER IF EXISTS prevent_invalid_verification_status_change ON verification_requests

CREATE TRIGGER prevent_invalid_verification_status_change
  BEFORE UPDATE ON verification_requests
  FOR EACH ROW
  EXECUTE FUNCTION prevent_invalid_verification_status_change()
