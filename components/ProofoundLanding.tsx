...
import { Menu, X, Check, ArrowRight, Shield, Users, Zap, Heart, Globe, Lock, TrendingUp, Eye, CheckCircle2, Clock, FileText, DollarSign, Bot, Puzzle, Recycle, Minimize, Sparkles, Network, Map, Brain, Key, Compass, BookOpen, Target, Lightbulb, Award, Crown } from 'lucide-react';
...
export function ProofoundLanding({ onGetStarted, onIndividualSignup, onOrganizationSignup }: ProofoundLandingProps) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo('.gsap-principle-card',
  ...
  return (
    <div ref={containerRef} className="relative bg-[#F7F6F1] dark:bg-[#1a1a1a] overflow-hidden">
      ...
      {showStickyProgress && (
        <StickyMiniCTA onGetStarted={handleGetStarted} />
      )}
...
function MinimalHeader({ menuOpen, setMenuOpen }: { menuOpen: boolean; setMenuOpen: (open: boolean) => void }) {
  return (
    <motion.header
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        >
          <img 
            ...
            className="w-7 h-7"
          />
          <span className="text-xl font-['Crimson_Pro'] text-[#1C4D3A] dark:text-[#D4C4A8]">Proofound</span>
        </motion.div>
        ...
        >
          {menuOpen ? <X className="w-5 h-5 text-[#1C4D3A] dark:text-[#D4C4A8]" /> : <Menu className="w-5 h-5 text-[#1C4D3A] dark:text-[#D4C4A8]" />}
        </button>
      </div>
      ...
      {menuOpen && (
        ...
        >
          <nav className="space-y-4">
            {['The Problem', 'How It Works', 'Principles', 'For Whom', 'Roadmap'].map((item, idx) => (
              <motion.a
                key={item}
...
// Section 1: Hero - The Promise
function HeroSection({ onGetStarted, shouldReduceMotion }: { onGetStarted?: () => void; shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });

  return (
    <section ref={ref} className="gsap-hero-section min-h-[85vh] flex items-center justify-center px-6 md:px-12 pt-24 pb-16 relative">
      <div className="gsap-hero-content relative z-10 max-w-4xl mx-auto text-center space-y-6">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-[#2D3330]/70 dark:text-[#D4C4A8]/70 max-w-2xl mx-auto">
            Unprecedented possibilities for work, business, and individual transformation. Backed by evidence, not vanity metrics.
          </p>
        </motion.div>
...
// Section 2: The Problem - Pains we solve
function ProblemSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });
  ...
  return (
    <section id="the-problem" ref={ref} className="gsap-problem-section flex items-center px-6 md:px-12 py-16 md:py-20 relative">
...
// Section 3: Our Answer - How Proofound works
function OurAnswerSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.2 });
  const scrollRef = useRef<HTMLDivElement>(null);

  const features = [
    { icon: Sparkles, title: "AI-powered matching", desc: "Evidence-based algorithms that connect you to aligned opportunities, not vanity metrics" },
    { icon: Award, title: "Proof-based profiles", desc: "Verified skills, impact stories, and credentials—all traceable and transparent" },
    { icon: Key, title: "Transferable verification", desc: "Once verified, your proofs travel with you across contexts and time" },
    { icon: Lock, title: "Granular privacy controls", desc: "You decide what's visible, to whom, and when—data dignity at every layer" },
    { icon: Eye, title: "Decluttered UX", desc: "Clean, Japandi-inspired design that respects your attention and mental space" },
    { icon: Heart, title: "Mental health tools", desc: "Built-in well-being support, not an afterthought—ikigai, safety planning, reflection" },
    { icon: Compass, title: "Life & career planning", desc: "Map your journey with purpose, not just the next job title" },
    { icon: TrendingUp, title: "Data democratization", desc: "Your data, your insights—we give you the tools to understand and control it" },
    { icon: Network, title: "Talent mobility", desc: "Skills and evidence are portable, opening doors across sectors and geographies" },
    { icon: BookOpen, title: "Education & guidance", desc: "Contextual learning pathways tailored to your goals and gaps" },
  ];

  return (
    <section id="how-it-works" ref={ref} className="px-6 md:px-12 py-16 md:py-20 relative overflow-hidden">
...
// Section 4: What makes it trustworthy - Guiding principles
function TrustworthySection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });
  ...
  return (
    <section id="principles" ref={ref} className="gsap-trustworthy-section flex items-center px-6 md:px-12 py-16 md:py-20 relative bg-gradient-to-b from-transparent to-[#1C4D3A]/5 dark:to-[#1C4D3A]/10">
      <div className="max-w-6xl mx-auto w-full">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : 30 }}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {principles.map((principle, idx) => (
            <motion.div
              ...
              animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : 20, scale: isInView ? 1 : 0.95 }}
...
// Section 5: Show, don't tell - Teasers of modules
function ModuleTeasersSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.2 });
  ...
  return (
    <section ref={ref} className="gsap-modules-section px-6 md:px-12 py-16 md:py-20 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : 30 }}
        ...
        <div className="relative -mx-6 md:-mx-12">
          ...
          <div className="relative overflow-x-auto scrollbar-hide pb-8 px-6 md:px-12">
            {/* Timeline Line */}
            <div className="absolute top-20 left-0 right-0 h-0.5 bg-gradient-to-r from-[#7A9278] via-[#5C8B89] to-[#C76B4A]" style={{ width: `${modules.length * 280}px` }} />
...
function PersonasSection({ 
  ...
  return (
    <section id="for-whom" ref={ref} className="flex items-center px-6 md:px-12 py-16 md:py-20 relative">
...
// Section 7: Why now - Timing & context
function WhyNowSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });

  const reasons = [
    { label: "AI and Technology advancements are shifting global paradigms", desc: "New tools demand new trust models and possibilities" },
    { label: "Simplicity of faking and growing trust challenges", desc: "Misinformation and deepfakes erode credibility everywhere" },
    { label: "Global mental health challenges grow as people question their immediate future and seek new transformation tools", desc: "Unprecedented uncertainty drives need for well-being infrastructure" },
    { label: "Obsoletion of outdated CV and recruitment standards that become less and less efficient", desc: "Traditional credentials don't capture real capability or values alignment" },
    { label: "Rapid globalization calls for coordination on an unprecedented scale", desc: "Cross-border collaboration requires new verification and trust systems" },
  ];

  return (
    <section ref={ref} className="flex items-center px-6 md:px-12 py-16 md:py-20 relative bg-gradient-to-b from-transparent to-[#C76B4A]/5 dark:to-[#C76B4A]/10">
...
// Section 8: Proof & credibility
function ProofSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });
        ...
        >
          <div>
            ...
            <p className="text-[#2D3330]/70 dark:text-[#D4C4A8]/70 mb-4">
              Every proof is traceable to its source. We use cryptographic signatures, time-stamped evidence, and transparent provenance chains to ensure authenticity.
            </p>
            <a href="#" className="text-[#1C4D3A] dark:text-[#6FAAA0] underline hover:no-underline">
          ...
          <div className="border-t border-[#1C4D3A]/10 dark:border-[#D4C4A8]/10 pt-8">
            ...
            <p className="text-[#2D3330]/70 dark:text-[#D4C4A8]/70 mb-4">
              Granular privacy controls at every layer. You decide what's visible, to whom, and when. We never sell your data or use it for purposes you haven't explicitly consented to.
            </p>
            <a href="#" className="text-[#1C4D3A] dark:text-[#6FAAA0] underline hover:no-underline">
          ...
          <div className="border-t border-[#1C4D3A]/10 dark:border-[#D4C4A8]/10 pt-8">
            ...
            <p className="text-[#2D3330]/70 dark:text-[#D4C4A8]/70">
              Our anti-bias algorithms are continuously monitored and audited. We publish transparency reports that anyone can understand—no technical jargon required.
            </p>
          </div>
        </motion.div>
      </div>
...
function StewardOwnershipSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });
  ...
  return (
    <section id="steward-ownership" ref={ref} className="flex items-center px-6 md:px-12 py-16 md:py-20 relative bg-gradient-to-b from-transparent to-[#7A9278]/5 dark:to-[#7A9278]/10">
      <div className="max-w-6xl mx-auto w-full">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : 30 }}
        ...
        >
          ...
          <p className="text-lg text-[#2D3330]/70 dark:text-[#D4C4A8]/70 max-w-3xl mx-auto">
            Steward ownership ensures that a company's purpose and independence are protected by giving control to stewards, not external shareholders.
          </p>
        </motion.div>
...
// Section 10: Products & Subscriptions
function ProductsSubscriptionsSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: false, amount: 0.3 });
  const [viewMode, setViewMode] = useState<'individual' | 'organization'>('individual');

  // Track billing cycle for each product individually
  const [productBillingCycles, setProductBillingCycles] = useState<{ [key: number]: 'monthly' | 'annual' }>({
    0: 'monthly',
    1: 'monthly',
    2: 'monthly',
    3: 'monthly',
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-base md:text-lg text-[#2D3330]/70 dark:text-[#D4C4A8]/70 max-w-4xl mx-auto mb-8">
            Proofound pledges a lifelong commitment never to monetize by creating disparity or selling exposure. The core tools that fulfill our mission will always remain free for humans. Our purpose is to enable people to do more — paid tools exist only to create pure positive value at no one else's expense. We are fully transparent about profit distribution, a cornerstone of our Steward Ownership model.
          </p>
              ...
              {viewMode === 'individual' && 'priceMonthly' in product ? (
                <div className="mt-auto">
                  ...
                  <div className="h-8 mb-3">
                    {'savings' in product && product.savings && productBillingCycles[idx] === 'monthly' && (
                  ...
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-3xl font-['Crimson_Pro'] text-[#1C4D3A] dark:text-[#D4C4A8]">
                      €{productBillingCycles[idx] === 'monthly' ? product.priceMonthly : product.priceAnnual}
                    </span>
...
// Section 11: Final pitch + Big CTA
function FinalCTASection({ onGetStarted, shouldReduceMotion }: { onGetStarted?: () => void; shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  ...
  return (
    <section ref={ref} className="flex items-center px-6 md:px-12 py-16 md:py-20 relative overflow-hidden">
      <div className="gsap-final-cta relative z-10 max-w-3xl mx-auto w-full text-center">
        ...
        >
          ...
          <p className="text-xl text-[#2D3330]/70 dark:text-[#D4C4A8]/70 max-w-2xl mx-auto">
            Be part of building a credibility infrastructure that respects your dignity, protects your privacy, and amplifies your impact.
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-sm text-[#2D3330]/50 dark:text-[#D4C4A8]/50">
            We respect your privacy. No spam, no data sold. <a href="#privacy" className="underline hover:no-underline">Read our privacy stance</a>.
          </p>
        </motion.form>
        ...
        >
          Organizations: <a href="#partner" className="text-[#1C4D3A] dark:text-[#6FAAA0] underline hover:no-underline">Partner with us</a> • 
          Learn more: <a href="#principles" className="text-[#1C4D3A] dark:text-[#6FAAA0] underline hover:no-underline ml-1">Read the principles</a>
        </motion.p>
      </div>
    </section>
  );
...
// Section 12: Final Quote Animation
function FinalQuoteSection({ shouldReduceMotion }: { shouldReduceMotion: boolean }) {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({
    target: ref,
  ...
  return (
    <section ref={ref} className="min-h-[70vh] flex items-center justify-center px-6 md:px-12 py-24 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          ...
          >
            ...
            <div className="space-y-2 text-sm text-[#2D3330]/70 dark:text-[#D4C4A8]/70">
              ...
              <p className="flex items-center gap-2">
                <span>✉️</span>
                <a href="mailto:hello@proofound.com" className="hover:text-[#1C4D3A] dark:hover:text-[#6FAAA0] transition-colors">
                  hello@proofound.com
                </a>
              </p>
            </div>
          ...
          >
            <h4 className="mb-6 font-['Crimson_Pro'] text-[#1C4D3A] dark:text-[#D4C4A8]">Platform</h4>
          ...
          >
            <h4 className="mb-6 font-['Crimson_Pro'] text-[#1C4D3A] dark:text-[#D4C4A8]">Resources</h4>
            <ul className="space-y-3">
