...
| OCR / CV extraction support                          | `src/lib/expertise/document-extraction-provider.ts`, `src/lib/expertise/gcp-cv-ocr-*`, `services/gcp-cv-ocr/*`, proof-artifact text extraction routes  | `tests/lib/start-from-cv.test.ts`, `tests/lib/proof-artifact-text-extraction.test.ts`, `tests/lib/gcp-cv-ocr-config.test.ts`, `tests/lib/gcp-cv-ocr-status.test.ts`, `tests/lib/document-extraction-provider.test.ts`, `tests/lib/gcp-cv-ocr-service.test.ts` | OCR remains server-side/config-gated; browser OCR disabled for launch; unavailable GCP creds block Start from CV when GCP OCR is enabled.                                   |
| Excluded legacy CV import / Gemini helpers           | `src/app/api/expertise/cv-import/wizard-*`, `src/lib/expertise/gemini/*`, `src/archive/non_launch_pages/admin/ai-spend/page.tsx`                       | `tests/lib/nlp-extractor.test.ts`, `tests/lib/gemini-reranker.test.ts`, `tests/lib/gemini-taxonomy-shortlist.test.ts`, `tests/api/admin/analytics-cv-import-spend-route.test.ts`, `tests/ui/admin-ai-spend-page.test.tsx`                                     | Archived CV import and admin AI spend surfaces remain outside launch; excluded Gemini reranker/taxonomy helpers do not create active employer-review AI.                    |
...
10. **The primary AI launch test command did not cover every AI-specific local suite.**
    ...
    - Regression: the expanded command now passes as a composite AI gate with 40 files / 293 tests across the default and archived-admin configs.
...
11. **Default-excluded AI guardrail suites could be missed by a path-only launch command.**
    ...
    - Regression: `npm run test:launch:ai` now proves 39 default-config files / 292 tests, then `tests/ui/admin-ai-spend-page.test.tsx` through the archived config.
...
| Check                           | Status                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `npm run test:launch:ai`        | PASS                     | Composite gate: 39 default-config files / 292 tests, plus archived admin AI boundary 1 file / 1 test through `vitest.archived.config.ts`. Vitest printed local websocket `EPERM` noise but all suites passed.                                                                                                                                                                                                                                                                                                            |
