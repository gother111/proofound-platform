...
10. **The primary AI launch test command did not cover every AI-specific local suite.**
    ...
    - Regression: the expanded command now passes as a composite AI gate with 41 files / 303 tests across the default and archived-admin configs.
...
11. **Default-excluded AI guardrail suites could be missed by a path-only launch command.**
    ...
    - Regression: `npm run test:launch:ai` now proves 40 default-config files / 302 tests, then `tests/ui/admin-ai-spend-page.test.tsx` through the archived config.
...
13. **Legacy CV-import archive proof was not wired into the AI launch gate.**
    ...
    - Regression: the focused archive/guardrail slice passes with 2 files / 18 tests, and the full `npm run test:launch:ai` now runs 40 default-config files / 302 tests plus the archived admin boundary.

## Checks Run

| Check                           | Status                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `npm run test:launch:ai`        | PASS                     | Composite gate: 40 default-config files / 302 tests, plus archived admin AI boundary 1 file / 1 test through `vitest.archived.config.ts`. Vitest printed local websocket `EPERM` noise and the expected archived admin `NEXT_HTTP_ERROR_FALLBACK;404` boundary stderr, but all suites passed.                                                                                                                                                                                                                            |
