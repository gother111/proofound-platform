...
Reason: all repo-local AI, mock-provider, privacy, lint/typecheck, exact production-build, Playwright-rendered, Codex in-app Browser UI checks, and live Gemini provider smoke against a freshly pulled Vercel production env snapshot passed after fixes. The system is not claimable as PERFECT because the currently deployed production launch-status endpoint is still blocked until production env changes are redeployed and fresh launch smoke evidence is generated.
...
| Privacy preflight                                    | `src/lib/ai/privacy-preflight.ts`, `src/app/api/ai/privacy-preflight/check/route.ts`, `src/app/api/portfolio/visibility/route.ts`                      | `tests/lib/privacy-preflight.test.ts`, `tests/lib/ai-redaction.test.ts`, `tests/api/privacy-preflight-route.test.ts`, `tests/api/portfolio-visibility-route.test.ts`, rendered API check                                                                                                                                 | Deterministic privacy flags work without model review; no full files/original filenames/signed URLs sent.                                                                   |
...
| JD-to-L4 assignment skill mapping                    | `src/app/api/expertise/jd-to-l4/route.ts`, `src/lib/ai/jd-parser.ts`, `src/components/organization/JDMapper.tsx`                                       | `tests/lib/ai-launch-guardrails.test.ts`, `tests/api/jd-to-l4-route.test.ts`, `npm run test:launch:ai`                                                                                                                                                                                                                   | Active assignment helper remains local-only; raw job-description text is not sent through legacy external provider code.                                                    |
...
10. **The primary AI launch test command did not cover every AI-specific local suite.**
    ...
    - Regression: the expanded command now passes as a composite AI gate with 44 files / 320 tests across the default and archived-admin configs.
...
11. **Default-excluded AI guardrail suites could be missed by a path-only launch command.**
    ...
    - Regression: `npm run test:launch:ai` now proves 43 default-config files / 319 tests, then `tests/ui/admin-ai-spend-page.test.tsx` through the archived config.
...
13. **Legacy CV-import archive proof was not wired into the AI launch gate.**
    ...
    - Regression: the focused archive/guardrail slice passes with 2 files / 18 tests, and the full `npm run test:launch:ai` now runs 43 default-config files / 319 tests plus the archived admin boundary.
...
15. **Active non-`/api/ai` AI-adjacent routes were underrepresented in the AI gate.**
    - Evidence: `/api/expertise/jd-to-l4` had local-parser guardrail coverage but no direct route coverage, and `/api/portfolio/visibility` had a privacy-preflight route test outside `test:launch:ai`.
    - Fix: added `tests/api/jd-to-l4-route.test.ts`, wired both that suite and `tests/api/portfolio-visibility-route.test.ts` into `test:launch:ai`, and required both paths in the AI launch-command drift sentinel.
    - Regression: the focused active-route slice passes with 3 files / 16 tests; the full AI gate now covers direct JD route auth/length/error/logging behavior and portfolio visibility privacy preflight blocking.

16. **Production Gemini key slots were malformed with literal quote characters.**
    - Evidence: freshly pulled Vercel production env showed `AI_GEMINI_PROD_API_KEY` and `AI_GEMINI_STAGING_API_KEY` configured with matching outer quotes. Direct non-secret probes returned `400 API key not valid` for the quoted values, while the same unquoted values returned `200` with `gemini-3.1-flash-lite`.
    - Fix: `resolveGeminiProviderApiKey` now normalizes accidentally quoted server-only Gemini key values before selecting the current key slots, without accepting browser-exposed `NEXT_PUBLIC_*GEMINI*KEY` values.
    - Regression: `tests/lib/ai-provider-gemini-client.test.ts` covers quoted prod and staging key normalization. A live provider smoke with the freshly pulled Vercel production env now passes with `gemini-3.1-flash-lite`.

17. **Current deployed production launch status is still blocked by stale deployment state.**
    - Evidence: protected `https://proofound.io/api/monitoring/launch-status` returned `503 blocked` on 2026-05-16. It reported the deployed runtime still using `aiConfiguredModel: gemini-3.1-flash-lite-preview`, `startFromCvBeta.blockers: ["guest_first_proof_scaffolding_not_approved"]`, and stale synthetic launch-smoke monitors from 2026-05-11.
    - Current env snapshot: a fresh Vercel production env pull already has `AI_MODEL_DEFAULT=gemini-3.1-flash-lite`, but `START_FROM_CV_GUEST_FIRST_PROOF_SCAFFOLDING_ENABLED` is present and empty, and the quoted AI key slots require either redeploying this normalization fix or repairing the env values directly.
    - Required follow-up: deploy/redeploy the production environment after explicit production-change approval, set `START_FROM_CV_GUEST_FIRST_PROOF_SCAFFOLDING_ENABLED=true` if Start from CV should remain enabled, then regenerate fresh launch smoke evidence and rerun `BASE_URL=https://proofound.io npm run launch:status`.

## Checks Run

| Check                                                 | Status             | Notes                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run test:launch:ai`                              | PASS               | Composite gate: 43 default-config files / 319 tests, plus archived admin AI boundary 1 file / 1 test through `vitest.archived.config.ts`. Vitest printed local websocket `EPERM` noise and the expected archived admin `NEXT_HTTP_ERROR_FALLBACK;404` boundary stderr, but all suites passed.                                                                                                        |
...
| `npm run ai:provider:smoke`                           | PASS               | Reran with a freshly pulled Vercel production env snapshot after adding quote normalization for server-only Gemini key values. Default model `gemini-3.1-flash-lite` was accepted, fallback was unset, preflight had database/provider key/monthly hard cap/production-like runtime satisfied, disabled mode stayed safe, and `.artifacts/ai-provider-smoke.json` schema v2 records `success: true`. |
| `BASE_URL=https://proofound.io npm run launch:status` | BLOCKED            | Protected live production endpoint was reachable with the server-only cron token and returned `503 blocked`: deployed runtime still reported stale preview model config, Start from CV scaffolding approval missing, and stale synthetic launch-smoke monitors from 2026-05-11. Endpoint live-refreshed `/`, `/login`, and `/api/health` successfully.                                               |
| Focused provider hardening tests                      | PASS               | `npm run test -- tests/lib/ai-provider-gemini-client.test.ts tests/lib/ai-provider-smoke-artifact.test.ts`: 2 files / 30 tests. Vitest printed the expected local websocket `EPERM` noise, but all tests passed.                                                                                                                                                                                     |
...
- **Live deployed production readiness**: still blocked. The live endpoint is reachable and gives useful dependency evidence, but the currently deployed runtime has not yet picked up the stable-model env value / quote-normalization fix, Start from CV scaffolding approval is not true in the fresh env snapshot, and synthetic launch-smoke monitors are stale.
- **Production mutation approval**: direct Vercel production env mutation was attempted only after user approval, but the escalation reviewer blocked it as requiring exact approval for the specific production secret and feature-flag changes. No production env values were changed by this run.
- **Fresh launch-smoke regeneration**: not rerun against production in this slice. Existing production launch status reports six stale synthetic-smoke monitors from 2026-05-11.

## Final Verdict

**NOT PERFECT** until the current production deployment is redeployed/rechecked and launch status returns ready with fresh smoke evidence. Within the feasible local and production-snapshot scope, the AI features now have no known relevant failing checks, broken rendered flows, Browser UI blocker, privacy leak, legacy direct external-provider bypass, model-staleness bug, OCR/Document AI governance gap, proof-artifact OCR route coverage gap, active non-`/api/ai` AI-route coverage gap, default-excluded redaction/admin coverage gap, legacy CV-import archive coverage gap, provider-smoke preflight false-positive gap, build-time type error, AI-specific test warning, or live Gemini provider-smoke failure remaining.
