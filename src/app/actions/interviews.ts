...
export async function getInterviews(params?: { status?: string; matchId?: string }) {
...
const INTERVIEW_POLICY_PRESETS = {
  startup: { scheduleWithinDays: 7, maxDurationMinutes: 30 },
  enterprise: { scheduleWithinDays: 14, maxDurationMinutes: 45 },
  volunteer: { scheduleWithinDays: 21, maxDurationMinutes: 30 },
  advanced: { scheduleWithinDays: 7, maxDurationMinutes: 60 },
} as const;
...
export async function scheduleInterview(input: z.input<typeof ScheduleInterviewSchema>) {
  ...
  try {
    ...
    if (interviewsForMatchError) {
      ...
      if (legacyExistingInterviewError) throw legacyExistingInterviewError;
    ...
    if (hasBlockingInterview) throw new Error('Interview already exists for this match');
    ...
    if (scheduledDate < now || scheduledDate > maxScheduleDate) {
      throw new Error(`Interview must be scheduled within ${activePolicy.scheduleWithinDays} days`);
    }

    if (durationMinutes > activePolicy.maxDurationMinutes) {
      throw new Error(
        `Interview duration cannot exceed ${activePolicy.maxDurationMinutes} minutes for ${data.policyPreset} policy`
      );
    }
    ...
    if (normalizedPlatform === 'manual') {
      if (!data.manualMeetingLink)
        throw new Error('Meeting link is required when using manual platform');
      ...
      if (integrationError || !videoIntegration) {
        throw new Error(
          `${data.platform === 'zoom' ? 'Zoom' : 'Google Meet'} not connected. Please connect your account first, or use "manual" to provide your own meeting link.`
        );
      }
        ...
        if (normalizedPlatform === 'zoom') {
          ...
          try {
            ...
            if (knownFailureResponse)
              throw new Error(knownFailureResponse.error || knownFailureResponse.message);
            throw refreshError;
          }
        }
      }
      ...
      if (normalizedPlatform === 'zoom') {
        ...
        for (const participantId of participantSet) {
          ...
          if (profile.user?.email) participantEmails.push(profile.user.email);
        }
    ...
    if (error.name === 'ZodError') {
      throw new Error('Invalid request data: ' + JSON.stringify(error.errors));
    }

    throw new Error(error instanceof Error ? error.message : 'Failed to schedule interview');
  }
}
