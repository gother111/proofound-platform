...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Insert check-in
    const [checkin] = await db.insert(wellbeingCheckins).values({
      userId: user.id,
      stressLevel,
      controlLevel,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (filtered.length >= 2) {
      ...
      const recentScore = recent.reduce((sum, c) => sum + ((5 - c.stressLevel) + c.controlLevel), 0) / recent.length;
      const olderScore = older.reduce((sum, c) => sum + ((5 - c.stressLevel) + c.controlLevel), 0) / older.length;
