  ...
  .refine(
    (data) => {
      ...
      const hasCustomSkill = !!(
        ...
        data.subcat_id &&
        data.l3_id &&
        data.custom_skill_name
      );
      return hasSkillCode || hasCustomSkill;
    },
    {
      message:
        'Must provide either skill_code or custom skill details (cat_id, subcat_id, l3_id, custom_skill_name)',
    }
  );
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Case 2: User-created custom skill
      ...
      skillIdToUse =
        validated.skill_id ||
        `custom-${validated.cat_id}-${validated.subcat_id}-${validated.l3_id}-${validated.custom_skill_name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
      isCustomSkill = true;
    ...
    if (existing) {
      return NextResponse.json({ error: 'Skill already exists in your profile' }, { status: 409 });
    }
