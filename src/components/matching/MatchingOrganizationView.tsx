  ...
  const handleReviewAction = async (matchId: string, action: ReviewAction) => {
    ...
    try {
      ...
      if (action === 'request_intro' && data.conversationId && data.introApproved) {
        toast.success('Mutual interest approved. Opening communications...');
        setTimeout(() => {
          router.push(
            ...
            {assignments.map((assignment) => {
              ...
              return (
                <button
                  ...
                  aria-label={`${assignment.role}. ${assignmentStatusLabel(assignment.status)}. ${count} proof submission${count !== 1 ? 's' : ''}.${updateBadge ? ` ${updateBadge}.` : ''}${isSelected ? ' Current assignment review flow.' : ''}`}
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center p-8 bg-white/50 border border-dashed border-proofound-stone/60 rounded-xl">
              ...
              <h3 className="text-base font-semibold text-proofound-charcoal">
                Select an assignment review flow
              </h3>
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-proofound-stone/60 bg-white/70 p-8 text-center">
              ...
              <p className="mt-2 max-w-md text-xs leading-5 text-muted-foreground">
                ...
                opening this review flow to submissions.
              </p>
              <Button
                asChild
