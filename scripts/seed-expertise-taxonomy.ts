...
async function seedL2Categories(
  ...
  for (const [l1Code, l2Categories] of l1ToL2Map.entries()) {
    ...
    for (const l2 of l2Categories) {
      ...
      const { error } = await supabase.from('skills_subcategories').insert({
        cat_id: catId,
        subcat_id: subcatId,
        slug,
  ...
  for (const [l1Code, l2Categories] of l1ToL2Map.entries()) {
    ...
    for (const l2 of l2Categories) {
      ...
      for (const l3Name of l2.l3Items) {
        ...
        const { error } = await supabase.from('skills_l3').insert({
          cat_id: catId,
          subcat_id: subcatId,
          l3_id: l3Id,
...
async function seedL4Skills(
  ...
  for (let i = 0; i < skills.length; i += BATCH_SIZE) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      taxonomyRecords.push({
        ...
        description_i18n: skill.description
          ...
          : { en: `${skill.name} skill` },
        tags: [
          skill.l1_code.toLowerCase(),
          l2Info.l2Code.toLowerCase(),
          skill.l3_name.toLowerCase().split(' ')[0],
        ],
        status: 'active',
      });
    }
    ...
    if (taxonomyRecords.length > 0) {
      const { error } = await supabase.from('skills_taxonomy').insert(taxonomyRecords);
