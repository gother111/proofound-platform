...
function parseVersionFromFile(fileName) {
...
async function main() {
  ...
  const dbVersionSet = new Set(dbResult.rows.map((row) => row.version));

  const fileNotApplied = localEntries.filter((entry) => !dbVersionSet.has(entry.version));
  const appliedMissingFile = dbResult.rows.filter((row) => !localVersionSet.has(row.version));
