...
async function main() {
  ...
  let localLink;
  try {
    localLink = await readLocalProjectLink();
  } catch {
    const projectId = process.env.VERCEL_PROJECT_ID;
    const projectName = process.env.VERCEL_PROJECT_NAME ?? canonicalProjectName;

    if (!projectId) {
      throw new Error(
        'Unable to read .vercel/project.json and VERCEL_PROJECT_ID is not set.'
      );
    }

    localLink = { projectId, projectName };
    console.log('Using VERCEL_PROJECT_ID/VERCEL_PROJECT_NAME from environment.');
  }
