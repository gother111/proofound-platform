...
## Launch Gate v1 (Strict, Blocking)

Run in this order for two-sided MVP release sign-off:
...
4. `npm run build`
5. `npm run test:a11y`
6. `npm run test:privacy:all`
7. `npm run test:e2e:critical`
8. `BASE_URL=http://localhost:3000 npm run perf:budgets`
9. `BASE_URL=http://localhost:3000 SUS_STUDY_COMPLETE=true npm run go:no-go`
10. `npm run db:audit:migrations`
11. `npm run vercel:preflight`

Runtime orchestration:

- Prefer `npm run gates:runtime` for steps 8 and 9.
- `gates:runtime` starts `next start`, waits for `/api/health`, runs perf/go-no-go, and always tears down the server process.

Blocking policy:

- Any non-zero exit code is a launch blocker.
- `test:e2e:critical` skips are treated as launch blockers until seeded credentials are configured.
- `test:privacy:all` missing-env failures are treated as launch blockers until `.env.test` is fully wired.
