...
> Superseded reference note added `2026-05-19`:
>
> - This report is preserved as historical scope-compliance evidence only.
> - Do not use its route counts, launch drift inventory, or archived-surface findings as current launch truth without checking newer route inventory and sweep evidence first.
> - Current route-surface and archive evidence lives in `src/lib/launch/surface-policy.ts`, `tests/api/launch-surface-inventory.test.ts`, `tests/api/launch-page-inventory.test.ts`, `tests/scripts/launch-gate-config.test.ts`, and `.artifacts/mvp-surface-sweep-2026-05-19/SURFACE_SWEEP.md`.
...
| Moderation and SUS survey APIs | `src/app/api/moderation/**`, `src/app/api/feedback/sus/**`, `src/app/api/surveys/sus/**`                                                                            | SUS survey UI/hooks/triggers removed from active source on 2026-05-20; moderation remains separate follow-up evidence | Archived outside locked MVP corridor              | `archive`      | These are evidence-only surfaces for follow-up retirement       |
