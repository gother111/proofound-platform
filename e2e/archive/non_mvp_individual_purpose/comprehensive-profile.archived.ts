 ...
 * - Proof-readiness checklist behavior
 */
...
test.describe('Profile - Profile Basics', () => {
  ...
  test('Profile hero does not expose banner image upload in MVP', async ({ page }) => {
    await navigateToProfile(page);

    const coverSection = page.locator('text=/Add cover image|Change cover image/i').first();
...
test.describe('Profile - Purpose Block', () => {
  ...
  test('Mission: add mission (≤300 chars) with visibility', async ({ page }) => {
    const mission =
      "To build software that makes a positive impact on people's lives and helps solve real-world problems.";
  ...
  test('Purpose block in matching: purpose signals stay out of Match Detail', async ({ page }) => {
    // Navigate to matching to verify retired purpose-fit signals stay hidden.
  ...
  test('Purpose block in matching: PAC contribution is not shown', async ({ page }) => {
    await page.goto('/app/i/matching');
    await page.waitForLoadState('networkidle');

    const pacContent = page.locator('text=/PAC|Purpose-Alignment/i');
    await expect(pacContent).not.toBeVisible();
  });
});
...
test.describe('Profile - Visibility & Boundary Controls', () => {
  ...
  test('Field-level visibility: set per field (public/link-only/match-only/private)', async ({
    page,
  }) => {
    try {
  ...
  test('Field-level visibility: visibility matrix shown', async ({ page }) => {
    ...
    if (hasSection) {
      ...
      const settingsButton = page
        .locator('button:has-text("Visibility"), button:has-text("Privacy")')
        .first();
  ...
  test('Preview as different audiences: public, link-only, match-only', async ({ page }) => {
    ...
    const previewButton = page
      .locator('button:has-text("Preview"), button:has-text("View As")')
      .first();
...
test.describe('Profile - Proof Readiness', () => {
  test.beforeEach(async ({ page }) => {
    await loginUser(page, TEST_USER.email, TEST_USER.password);
    await navigateToProfile(page);
  });

  test('Proof readiness: checklist language is shown when available', async ({ page }) => {
    const hasChecklist = await verifyProofReadinessChecklist(page);

    expect(typeof hasChecklist === 'boolean').toBeTruthy();
  });

  test('Proof readiness: no profile scoring progress bar is required', async ({ page }) => {
    const scoreText = page.locator('text=/% complete|Profile .*%/i').first();
    const hasScoreText = await scoreText.isVisible().catch(() => false);

    expect(hasScoreText).toBe(false);
  });

  test('Proof readiness: next proof actions can be surfaced', async ({ page }) => {
    const nextActions = page.locator('text=/Next Best|Proof Readiness|Start proof|Review proof/i');
    const hasActions = await nextActions.isVisible().catch(() => false);

    expect(typeof hasActions === 'boolean').toBeTruthy();
  });

  test('Proof readiness: activation threshold indicator can appear', async ({ page }) => {
