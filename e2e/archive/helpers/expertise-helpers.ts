...
export async function getL1DomainCards(page: Page) {
  return page.locator('[data-testid*="l1"], [class*="L1Card"], [class*="domain-card"]');
}
...
export async function verifyL2CategoriesShown(page: Page) {
  const l2Categories = page.locator('[data-testid*="l2"], [class*="L2"], [class*="category"]');
...
export async function openAddSkillDrawer(page: Page) {
  const addButton = page
    .locator('button:has-text("Add"), button:has-text("Add Skill"), [data-testid="add-skill"]')
    .first();

  if (await addButton.isVisible()) {
    await addButton.click();
    await page.waitForSelector('text=/Add Skill|Select Skill|Choose Skill/i', { timeout: 5000 });
  } else {
...
export async function setSkillLevel(page: Page, level: number) {
  const levelInput = page
    .locator(`input[type="range"][value="${level}"], input[name*="level"], select[name*="level"]`)
    .first();

  if (await levelInput.isVisible()) {
    if (levelInput.evaluate((el) => el.tagName === 'SELECT')) {
...
export async function attachProof(page: Page, type: 'file' | 'link', value: string) {
  if (type === 'file') {
    ...
    const linkInput = page
      .locator('input[type="url"], input[placeholder*="link"], input[placeholder*="URL"]')
      .first();
...
export async function setSkillRecency(page: Page, date: Date) {
  const dateInput = page
    .locator('input[type="date"], input[name*="lastUsed"], input[name*="recency"]')
    .first();
...
export async function saveSkill(page: Page) {
  const saveButton = page
    .locator('button:has-text("Save"), button:has-text("Add Skill"), button:has-text("Create")')
    .first();
...
export async function openCVImport(page: Page) {
  ...
  if (await importButton.isVisible()) {
    ...
    await page.waitForSelector('text=/Import|Paste|CV|Job Description/i', { timeout: 5000 });
  }
}
...
export async function verifyCVSuggestions(page: Page): Promise<boolean> {
  const suggestions = page.locator('text=/suggested|mapped|why it mapped/i');
...
export async function acceptCVSuggestion(page: Page, suggestionIndex = 0) {
  ...
  const acceptButton = suggestion
    .locator('button:has-text("Accept"), button:has-text("Add")')
    .first();
...
export async function requestSkillVerification(
  ...
  if (await verifyButton.isVisible()) {
    ...
    const submitButton = page
      .locator('button:has-text("Send"), button:has-text("Request")')
      .first();
...
export async function checkEmptyState(page: Page): Promise<boolean> {
  const emptyState = page.locator('text=/no skills|get started|add your first skill/i');
