...
describe('MatchingOrganizationView', () => {
  ...
  it('exposes selected assignment, review tabs, and selected proof submission semantics', async () => {
    ...
    const assignmentButton = screen.getByRole('button', {
      name: /Field operations launch lead.*Current assignment review flow/i,
    });
  ...
  it('keeps active empty assignments in the proof-submission waiting state', async () => {
    ...
    expect(
      screen.getByText(
        "We are actively processing submissions against your assignment's proof requirements. Keep this review flow open."
      )
    ).toBeInTheDocument();
    expect(
  ...
  it('brings selected proof details into view on mobile submission selection', async () => {
    ...
    expect(screen.getByText('Strongest proof record')).toBeInTheDocument();
    expect(
      screen.getByText(/The summary uses scoped Proof record signals, reason codes/i)
    ).toBeInTheDocument();
